...
Jacobi::Jacobi(Matrix<double> A, Matrix<double> x, Matrix<double> b)
	: A(A), x(x), b(b), interactions(7) {
	// TODO Auto-generated constructor stub
}

void Jacobi::setInteractions(int k) {
	interactions = k;
}

Matrix<double> Jacobi::exec() {
	int k = 0;
	Matrix<double> Xold;

	while (k < interactions) {
		Xold.set_all(x.rows(), x.cols(), x.get_all());
		for (int i = 0; i < A.rows(); i++) {
			double soma = b.get(i, 0);
			for (int j = 0; j < A.cols(); j++) {
				if (i != j) {
					soma -= A.get(i, j) * Xold.get(j, 0);
				}
			}
			soma /= A.get(i, i);
			x.set(i, 0, soma);
